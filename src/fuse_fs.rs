...
#[derive(Clone, Debug)]
enum NodeKind {
    Dir { children_loaded: bool },
    File { size: u64, etag: Option<String>, mtime: SystemTime },
}
...
impl BlobFs {
    pub fn new(mount: MountConfig, blob: Arc<BlobClient>, fetcher: Arc<Fetcher>, rt: Handle) -> Self {
        let mut by_ino = HashMap::new();
        let by_parent_name = HashMap::new();
        let mut children = HashMap::new();
        by_ino.insert(ROOT_INO, Node {
            ino: ROOT_INO,
            parent: ROOT_INO,
            name: "".into(),
            full_path: "".into(),
            kind: NodeKind::Dir { children_loaded: false },
        });
    ...
    fn attr_for(node: &Node, uid: u32, gid: u32) -> FileAttr {
        match &node.kind {
            ...
            NodeKind::File { size, mtime, .. } => FileAttr {
                ino: node.ino, size: *size, blocks: (*size + 511) / 512,
                atime: *mtime, mtime: *mtime, ctime: *mtime, crtime: *mtime,
        ...
        let need_load = {
            ...
            match n.kind {
                NodeKind::Dir { children_loaded } => {
                    let recent = self.last_listing.get(&ino)
                        .map(|t| t.elapsed() < self.listing_cache_ttl).unwrap_or(false);
                    !children_loaded || !recent
                }
                _ => return Err(ENOTDIR),
            }
        };
        if !need_load { return Ok(()); }
        let prefix = {
            let g = self.inner.read().await;
            let n = g.by_ino.get(&ino).ok_or(ENOENT)?;
            let mut p = if self.mount.prefix.is_empty() { String::new() } else { format!("{}/", self.mount.prefix.trim_matches('/')) };
            if !n.full_path.is_empty() { p.push_str(&n.full_path); p.push('/'); }
            p
        };
        let pref_arg = if prefix.is_empty() { None } else { Some(prefix.as_str()) };
        let (blobs, prefixes) = self.blob.list_blobs(&self.mount.account, &self.mount.container, pref_arg, false)
            .await.map_err(|e| { tracing::error!(error=%e, "list_blobs failed"); EIO })?;
    ...
    fn upsert_dir(&self, g: &mut Inner, parent: u64, name: &str, full_blob_path: &str) {
        if let Some(&existing) = g.by_parent_name.get(&(parent, name.to_string())) {
            let _ = existing; return;
        }
        let ino = g.next_ino; g.next_ino += 1;
        let trim = full_blob_path.trim_end_matches('/');
        let strip_prefix_len = if self.mount.prefix.is_empty() { 0 } else { self.mount.prefix.trim_matches('/').len() + 1 };
        let full_path = trim.get(strip_prefix_len..).unwrap_or("").to_string();
        let node = Node {
            ino, parent, name: name.to_string(),
            full_path,
            kind: NodeKind::Dir { children_loaded: false },
        };
    ...
    fn upsert_file(&self, g: &mut Inner, parent: u64, name: &str, full_blob_name: &str, size: u64, etag: Option<String>, mtime: SystemTime) {
        if let Some(&existing) = g.by_parent_name.get(&(parent, name.to_string())) {
            if let Some(n) = g.by_ino.get_mut(&existing) {
                if let NodeKind::File { size: s, etag: e, mtime: m } = &mut n.kind {
                    *s = size; *e = etag; *m = mtime;
                }
            }
            return;
        }
        let strip_prefix_len = if self.mount.prefix.is_empty() { 0 } else { self.mount.prefix.trim_matches('/').len() + 1 };
        let full_path = full_blob_name.get(strip_prefix_len..).unwrap_or(full_blob_name).to_string();
        let ino = g.next_ino; g.next_ino += 1;
        let node = Node {
            ino, parent, name: name.to_string(), full_path,
...
impl Filesystem for BlobFs {
    fn lookup(&mut self, req: &Request, parent: u64, name: &OsStr, reply: ReplyEntry) {
        ...
        let uid = req.uid(); let gid = req.gid();
        self.rt.spawn(async move {
            if let Err(e) = me.ensure_dir_listed(parent).await { reply.error(e); return; }
            ...
            if let Some(p) = g.by_ino.get(&parent) {
                let prefix = if me.mount.prefix.is_empty() { String::new() } else { format!("{}/", me.mount.prefix.trim_matches('/')) };
                let candidate = if p.full_path.is_empty() { format!("{prefix}{name}") }
                                else { format!("{prefix}{}/{name}", p.full_path) };
                drop(g);
                match me.blob.get_blob_properties(&me.mount.account, &me.mount.container, &candidate).await {
                    Ok(info) => {
                        let mut g = me.inner.write().await;
                        me.upsert_file(&mut g, parent, &name, &candidate, info.content_length, info.etag, SystemTime::now());
    ...
    fn getattr(&mut self, req: &Request, ino: u64, _fh: Option<u64>, reply: ReplyAttr) {
        ...
        self.rt.spawn(async move {
            ...
            match g.by_ino.get(&ino) {
                Some(n) => { let attr = BlobFs::attr_for(n, uid, gid); reply.attr(&TTL, &attr); }
            ...
            let (full_path, file_size) = {
                ...
                match g.by_ino.get(&ino) {
                    Some(n) => match &n.kind {
                        ...
                        _ => { reply.error(libc::EISDIR); return; }
                    },
                    None => { reply.error(ENOENT); return; }
                }
            };
            if (offset as u64) >= file_size { reply.data(&[]); return; }
            let length = (size as u64).min(file_size - offset as u64);
            fuse_reads.fetch_add(1, Ordering::Relaxed);
            match me.fetcher.fetch_range(&me.mount, &full_path, offset as u64, length, file_size).await {
                Ok(b) => {
    ...
    fn readdir(&mut self, _req: &Request, ino: u64, _fh: u64, offset: i64, mut reply: ReplyDirectory) {
        let me = self.clone_handles();
        self.rt.spawn(async move {
            if let Err(e) = me.ensure_dir_listed(ino).await { reply.error(e); return; }
            let g = me.inner.read().await;
            ...
            if let Some(kids) = g.children.get(&ino) {
                for &k in kids {
                    if let Some(n) = g.by_ino.get(&k) {
                        let kind = match n.kind { NodeKind::Dir{..} => FileType::Directory, NodeKind::File{..} => FileType::RegularFile };
