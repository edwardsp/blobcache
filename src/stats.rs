...
impl Stats {
    pub fn new() -> Arc<Self> {
        ...
        let cache_evictions = IntCounter::new("blobcache_cache_evictions_total", "cache evictions").unwrap();
        let cache_inserts = IntCounter::new("blobcache_cache_inserts_total", "cache inserts").unwrap();
        ...
        let blob_fetch_bytes = IntCounter::new("blobcache_blob_fetch_bytes_total", "blob fetched bytes").unwrap();
        let peer_fetches_ok = IntCounter::new("blobcache_peer_fetches_ok_total", "peer fetches ok").unwrap();
        let peer_fetches_miss = IntCounter::new("blobcache_peer_fetches_miss_total", "peer fetch misses").unwrap();
        let peer_fetches_err = IntCounter::new("blobcache_peer_fetches_err_total", "peer fetch errors").unwrap();
        ...
        let joins = IntCounter::new("blobcache_cluster_joins_total", "peer joins observed").unwrap();
        let failures = IntCounter::new("blobcache_cluster_failures_total", "peer failures observed").unwrap();
        let config_mismatches = IntCounter::new("blobcache_cluster_config_mismatches_total", "config hash mismatches").unwrap();
        let members_alive = IntGauge::new("blobcache_cluster_members_alive", "alive members").unwrap();
...
pub async fn serve(stats: Arc<Stats>, addr: std::net::SocketAddr,
                   cache: Arc<crate::cache::DiskCache>,
                   membership: crate::cluster::Membership) -> crate::error::Result<()> {
    use bytes::Bytes;
            ...
            let _ = http1::Builder::new().serve_connection(io, service_fn(move |req: Request<Incoming>| {
                ...
                async move {
                    let resp = match (req.method(), req.uri().path()) {
                        (&Method::GET, "/metrics") => {
                            let cs = &cache.stats;
                            s.cache_hits.reset(); s.cache_hits.inc_by(cs.hits.load(Ordering::Relaxed));
                            s.cache_misses.reset(); s.cache_misses.inc_by(cs.misses.load(Ordering::Relaxed));
                            s.cache_evictions.reset(); s.cache_evictions.inc_by(cs.evictions.load(Ordering::Relaxed));
                            s.cache_inserts.reset(); s.cache_inserts.inc_by(cs.inserts.load(Ordering::Relaxed));
                            s.cache_bytes.set(cs.bytes_in_use.load(Ordering::Relaxed) as i64);
                            let all = membership.members_all();
                            let alive = all.iter().filter(|n| matches!(n.state, crate::cluster::NodeState::Alive)).count();
                            let dead = all.iter().filter(|n| matches!(n.state, crate::cluster::NodeState::Dead)).count();
                        ...
                        (&Method::GET, "/peers") => {
                            let v = serde_json::json!({"members": membership.members_all()});
                            let body = serde_json::to_vec(&v).unwrap();
                            Response::builder().status(200)
                                .header("content-type", "application/json")
                                .body(Full::new(Bytes::from(body))).unwrap()
